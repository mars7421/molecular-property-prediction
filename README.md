...
본 프로젝트는 분자 구조 기반 feature를 활용하여 **용해도(solubility)** 를 예측하는 것을 목표로 합니다.

단순 상관관계 분석은 변수 간 복잡한 관계를 충분히 설명하지 못하며,
특히 변수 간 상호작용과 비선형 패턴이 존재할 경우 한계가 있습니다.

이에 따라 다양한 feature 조합과 머신러닝 모델을 비교하여,
예측 성능에 가장 효과적인 변수 구성과 모델을 도출하고자 하였습니다.
...
이는 변수 간 상호작용 및 비선형 관계가 존재할 가능성을 시사함
...
## 📊 Results
- 각 feature 구성과 모델에 따른 성능을 비교한 결과는 다음과 같음  
...
- 모델 성능 향상의 원인을 분석하기 위해 feature importance 확인함  

![feature importance](screenshots/feature_importance.png)

* Molecular Weight, Polar Surface Area 등 주요 변수의 높은 중요도 확인
* 일부 변수는 상관관계 대비 중요도가 낮게 나타나 정보 중복 가능성 확인
* 단일 변수보다 변수 간 조합이 예측에 더 큰 영향을 미침을 확인
...
👉 단순히 많은 feature를 사용하는 것이 아니라,  
   데이터 특성에 맞는 feature 선택과 모델 구조가  
   예측 성능을 결정하는 핵심 요소임을 실험적으로 검증함
